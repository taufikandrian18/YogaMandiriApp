...
namespace TestCoba
{
    public class ITextEventsPnj : PdfPageEventHelper
    {
        ...
        {
            DataRow drMst = GetData("SELECT * FROM POPenjualanMst a INNER JOIN Customer b on a.IdCustomer = b.IdCustomer WHERE IdPOPenjualanMst = " + parametro).Rows[0];
            DataTable drDtl = GetData("SELECT B.BarangItemCode,B.NamaBarang,A.QtyBarangPOPenjualanDtl,A.JumlahBoxPOPenjualanDtl,A.HargaPOPenjualanDtl,A.TotalHargaPOPenjualanDtl FROM POPenjualanDtl A INNER JOIN Barang B ON A.IdBarang = B.IdBarang WHERE A.IdPOPenjualanMst = " + parametro);
            ...
            Phrase p1Header = new Phrase("Yoga Mandiri Utama", baseFontBig);
